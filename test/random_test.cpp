...
TEST(random_test, plotting) {
    fstream bilad_file("bilad.csv", fstream::out),
        exact_bilad_file("exact_bilad.csv", fstream::out);
    ...
    for (size_t nodes = 20; nodes <= 2000; nodes += 20) {
        ...
        for (size_t i = 0; i < repeat_times; ++i) {
            ...
            auto t1 = chrono::high_resolution_clock::now();
            auto [p_plus, p_minus, lambda, flag, bilad_count] =
                bilad(g, 0, nodes - 1, delta);
            auto t2 = chrono::high_resolution_clock::now();
            bilad_time +=
        ...
        bilad_file << nodes << ',' << bilad_dijkstra / repeat_times << ','
                   << bilad_time / repeat_times << '\n';
        exact_bilad_file << nodes << ',' << exact_bilad_dijkstra / repeat_times
                         << ',' << exact_bilad_time / repeat_times << '\n';
    }
}

TEST(random_test, plotting_degree) {
    fstream bilad_file("bilad_degree.csv", fstream::out),
        exact_bilad_file("exact_bilad_degree.csv", fstream::out);
    ...
    for (size_t degree = 1; degree <= 200; degree += 1) {
        ...
        bilad_file << degree << ',' << bilad_dijkstra / repeat_times << ','
                   << bilad_time / repeat_times << '\n';
        exact_bilad_file << degree << ',' << exact_bilad_dijkstra / repeat_times
                         << ',' << exact_bilad_time / repeat_times << '\n';
    }
}

TEST(random_test, hist) {
    fstream bilad_file("bilad_hist.csv", fstream::out),
        exact_bilad_file("exact_bilad_hist.csv", fstream::out);
    ...
    cout << fixed << setprecision(5) << "gap rate: " << gap / repeat_times
         << endl;
}
