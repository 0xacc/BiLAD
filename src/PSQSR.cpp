...
namespace BiLAD {
    tuple<GraphPath, GraphPath, size_t> PSQSR(const Graph &graph, size_t src, size_t dest, weight_t delta) {
        ...
        for (size_t i = 0; i < graph.size(); ++i) {
            for (size_t j = 0; j < graph.size(); ++j) {
                for (size_t k = 0; k < graph[j].size(); ++k) {
                    ...
                    auto x_v = c_dist[v];
                    auto y_v = d_dist[v];
                    if (isinf(x_u) || isinf(y_u))continue;
                    ...
                    } else if (graph.d_dist(p_c) > delta && graph.d_dist(p_d) < delta) {
