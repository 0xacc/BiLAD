...
std::tuple<GraphPath, GraphPath> biweight_dijkstra(const Graph& graph,
                                                   ...
                                                   double lambda, double gama) {
    size_t n = graph.size();

    vector<bool> visited(n, false);
    vector<double> dist(n, numeric_limits<double>::max());
    dist[src] = 0;
    vector<weight_t> c_dist(n, numeric_limits<weight_t>::max()),
        d_dist(n, numeric_limits<weight_t>::max());
    c_dist[src] = 0;
    d_dist[src] = 0;
    using Dist = pair<size_t, double>;
    auto cmp = [](const Dist& lhs, const Dist& rhs) {
