...
namespace BiLAD{
std::tuple<GraphPath, GraphPath> biweight_dijkstra(const Graph &graph, size_t src, size_t dest, double lambda,double gama) {
    size_t n = graph.size();
    ...
    while(c_prev[i]!=src) { c_path.push_back(c_prev[i]);i=c_prev[i]; }
    c_path.push_back(src);
    reverse(c_path.begin(),c_path.end());

    size_t j=dest;
    d_path.push_back(dest);
    while(d_prev[j]!=src) { d_path.push_back(d_prev[j]);j=d_prev[j]; }
