...
bilad(const Graph& graph, size_t src, size_t dest, weight_t delta) {
    ...
    if (delay_p_minus > delta)
        return {{}, {}, -1, flag, dijkstra_count};
    if (delay_p_minus == delta)
        return {{}, p_minus, -1, flag, dijkstra_count};
    ...
    while (abs(theta_up - theta_down) > EPSILON) {
        ...
        lambda = static_cast<double>(cost_p_minus - cost_p_plus) /
                 (delay_p_plus - delay_p_minus);
